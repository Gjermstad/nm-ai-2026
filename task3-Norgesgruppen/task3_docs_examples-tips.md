...
Using `YOLOv8n` with GPU auto-detection.

Important: The pretrained COCO model outputs COCO class IDs (`0-79`), not product IDs (`0-355`). For correct product classification, fine-tune on the competition training data with `nc=357`. Detection-only submissions (wrong `category_id`s) still score up to `70%`.
