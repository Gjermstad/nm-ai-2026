...
Always enforce a minimum probability floor of `0.01` per class, then renormalize so the values still sum to `1.0`:
...
- Per-seed scores are averaged to compute `round_score`
