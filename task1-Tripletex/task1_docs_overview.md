...
- Your agent reads the prompt, optionally processes attached files (`PDF`s, images)
...
## Comments from the NM I AI Slack channels
- task 11 on Tier 2 is one of the harder ones, many have trouble with it but some are scoring points on it
- Friday: Some are complaining that "FYI: We have experienced getting invalid tripletex session_token at least 3 times in the last few hours from the task dispatcher :white_frowning_face: `ERROR - API error 403: {"error":"Invalid or expired token"}`"
- "Tripetex konto før solve, skal company egentlig væer satt opp med kontonummer? Jeg får feil på opprettelse av faktura fordi bankkontonummer ikke er satt på company. Er dette en reell bug eller mening at det skal finnes en workaround. Det kommer jo ikke kontonummer i oppgaven." Several are saying they experience the same, but the organizers says it is answered in the docs.
- How many quotas are we supposed to have today? 10 per task / 180